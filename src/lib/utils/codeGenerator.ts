...
function generateTypeScriptCode(
  ...
  if (provider === 'openclaw') {
    return generateOpenClawTypeScriptCode(buildId, timestamp, config, baseUrl)
  } else if (provider === 'openai') {
    return generateOpenAITypeScriptCode(buildId, timestamp, config)
  } else {
    // Default to Anthropic
    return generateAnthropicTypeScriptCode(buildId, timestamp, config)
  }
}
...
function generateEnvExample(buildState: BuildState): string {
  ...
  if (provider === 'openclaw') {
    return `OPENCLAW_API_KEY=your_api_key_here
`
  } else if (provider === 'openai') {
